"""
    express news spider for east money
"""
...
class _ExpressListFetcher:
    """
        get express items list by category id
    """
    URL = 'http://newsapi.eastmoney.com/kuaixun/v2/api/list?callback=jQuery22104129033022482793_1544004029553&column=%s&p=1&limit=50&_=1544004029554'
...
# 快讯
class DfcfExpressSpider(spider.Spider):
    def __init__(self, id, name, cron):
        ...
        :param cron:
        """
        self._fetchers = [_ExpressListFetcher(category) for category in config.express.categories]
        ...
        try:
            ...
            for fetcher in self._fetchers:
                ...
                if items is not None:
                    ...
                    items = stores.news.distinct(config.SITE, items, lambda item: item['id'])

                    # process each news item detail
                    newitems = []
                    for item in items:
                        # fetch news detail data
                        detail = await _DetailFetcher(item['id']).fetch()
            ...
            tornado.log.app_log.info('spider-%s-express: category(total: %s, updated: %s), news(total: %s, updated: %s)', config.SITE, category_total, category_updated, news_total, news_updated)
        except Exception as e:
            tornado.log.app_log.error('spider-%s-express: %s', config.SITE, str(e))
